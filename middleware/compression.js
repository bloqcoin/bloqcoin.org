...
let sanitizeOptions = require(path.join(__dirname, './util/options.js')).sanitizeOptions;
let findEncoding = require(path.join(__dirname, './util/encoding-selection.js')).findEncoding;
 ...
 */
function expressStaticGzipMiddleware(options) {
	...
	function expressStaticGzip(req, res, next) {
		...
		changeUrlFromDirectoryToIndexFile(req, res);
	...
	function changeUrlFromDirectoryToIndexFile(req, res) {
		...
		if (!path.extname(req.url) && req.url !== '/') {
			...
			if (segments.length > 2) {
				...
				req.url = `${segments[1]}_${segments[2]}.html`;
			}
			else {
